 ...
 * fonts resolve to real loaded families.
 */
...
import { brandColors, brandFonts, brandRadii, brandShadow } from "../tokens";
...
describe("brand palette (site design truth)", () => {
	...
	it("uses the site radii scale 10/16/28 + pill", () => {
		expect(brandRadii).toEqual({ lg: 28, md: 16, pill: 999, sm: 10 });
	});
});

describe("brand shadows are soft, not the legacy hard offset", () => {
	it.each(Object.entries(brandShadow))(
		"%s blurs with partial opacity",
		(_name, s) => {

...
function parseArgs(argv: string[]): Args {
  ...
  if (!filePath) {
    throw new Error(
      "Usage: bun --env-file=.env run scripts/directory-import/import.ts <file.json> [--dry-run]",
    );
  }
...
async function loadJson(
  filePath: string,
): Promise<TopLevelResponse & { usr: DirectoryRecord[] }> {
...
function prepareRecords(records: DirectoryRecord[]): {
  prepared: Prepared[];
  skipped: number;
} {
  ...
  for (const record of records) {
    ...
    if (!business) {
      skipped += 1;
      console.warn(
        `  skip: invalid record (missing uid/nam) — uid=${record.uid ?? "(none)"}`,
      );
      continue;
    }
...
async function main(): Promise<void> {
  ...
  console.log(
    `Records: ${json.usr.length} | valid: ${prepared.length} | skipped: ${skipped}`,
  );

  if (dryRun) {
    console.log("--- DRY RUN (no database writes) ---");
    console.log(
      `Batch: source_type=${batchPayload.source_type} count=${batchPayload.source_record_count}`,
    );
    console.log(
      "Sample business:",
      JSON.stringify(prepared[0]?.business, null, 2),
    );
  ...
  const businessRows = prepared.map((p) => ({
    ...p.business,
  ...
  // 3) Replace phone rows atomically. Build the replacement set, de-duplicating
  //    by (business_id, phone_number) so a duplicate in the source can't violate
  //    the unique constraint mid-insert. The delete + insert run in a single
  //    transaction inside `directory_replace_phones`, so a failure can never
  //    leave a business with no phones (see migration 0006).
  const seen = new Set<string>();
  const phoneRows = prepared.flatMap((p) => {
    const businessId = idBySourceUid.get(p.business.source_uid);
    if (!businessId) return [];
    return p.phones.flatMap((phone) => {
      const key = `${businessId}|${phone.phone_number}`;
